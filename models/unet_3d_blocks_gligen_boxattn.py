...
def roi_align_from_boxes(features, normalized_rois, masks, output_size, num_frame):
    ...
    # Create a tensor of indices for each batch: [bs, num_frame*20]
    batch_indices = torch.arange(bs).view(-1, 1).float().to(rois.device).repeat(1, num_frame*20)
    ...
    flat_batch_indices = batch_indices.view(-1, 1)
    ...
    valid_batch_indices = flat_batch_indices[flat_masks == 1]
    valid_frame_indices = flat_frame_indices[flat_masks == 1].long().squeeze(-1)
    
    query_masks = torch.zeros(num_frame, h, w)
    ...
    final_attn_mask = torch.full((num_frame*h*w, roi_masks.shape[0]), -10000.0, dtype=torch.float)
    ...
    return output.flatten(2, 3), final_attn_mask.view(num_frame, -1, roi_masks.shape[0])
...
class CrossAttnDownBlock3D(nn.Module):
    def __init__(
        ...
        self.register_parameter("alpha_box_attn", nn.Parameter(torch.tensor(0.0)))
    ...
    def forward(
        ...
        for resnet, temp_conv, attn, temp_attn, tracklet_attn in zip(
            self.resnets, self.temp_convs, self.attentions, self.temp_attentions, self.tracklet_attentions
        ):
            ...
            if self.gradient_checkpointing:
                ...
                if env_var_value == "enable":
                    ...
                    hidden_states = tracklet_attn(
                        ...
                        cross_attention_kwargs=None,
                        attention_mask=final_attn_mask.to(hidden_states).repeat(hidden_states.shape[0]//num_frames, 1, 1),
                    ).sample
