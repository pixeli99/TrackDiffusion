...
class TextToVideoSDPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    ...
    def __call__(
        ...
        if bbox_prompt is not None:
            ...
            tokenizer_inputs = self.tokenizer(seg_phrases, padding=True, return_tensors="pt").to(device)
            # For the token, we use the same pre-trained text encoder
            # to obtain its text feature
            _text_embeddings = self.text_encoder(**tokenizer_inputs).pooler_output # [8 * 15, 1024]
            _text_embeddings = _text_embeddings.view(num_frames, fixed_objs, -1) # [8, 15, 1024]
            masks = video_masks

            # 8, 15, 768
            text_embeddings = _text_embeddings.unsqueeze(0)
            # 8, 15
            masks = masks.unsqueeze(0).to(text_embeddings)
            # 8, 15, 4
            boxes = bbox_prompt.unsqueeze(0).to(text_embeddings)
            ...
            cross_attention_kwargs["gligen"] = {"boxes": boxes.flatten(1, 2),
